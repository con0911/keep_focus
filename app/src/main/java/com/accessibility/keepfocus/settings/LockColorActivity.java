...
public class LockColorActivity extends Activity implements OnClickListener {
    ...
    private String choosePassColor, confirmPassColor;
    private boolean confirmColor = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*
         * Set enable for button
        */
        getActionBar().setTitle("SET PIN CODE");
        changeDisplayPassword(mPinCodeNumber);
        if (myPass.getString("PinCode", "") != "") {
            havePass = true;
            text_Guide.setText(getString(R.string.confirm_pin_code));
        }
    }
    ...
    void initializeIdentifiers() {
        ...
        text_Guide.setText(getApplicationContext().getText(
        ...
        if (number == 4) {
            ...
            if (havePass) {
                if(mLockPinCode.equals(myPass.getString("PinCode", ""))) {
                    havePass = false;
                    delayResetPass();
                    text_Guide.setText(getString(R.string.input_new_pin_code));
                } else {
                ...
                if (!confirmColor) {
                    confirmColor = true;
                    choosePassColor = mLockPinCode;
                    delayResetPass();
                } else {
                    confirmPassColor = mLockPinCode;
                    if (confirmPassColor.equals(choosePassColor)) {
                        Toast.makeText(this, "Password Accept! Done!",
                                Toast.LENGTH_LONG).show();
                        confirmColor = false;
                        SharedPreferences.Editor e = myPass.edit();
                        e.putString("PinCode", mLockPinCode);
                        e.commit();
                        finish();
                    } else {
                        Toast.makeText(this, "Wrong Password! Try again",
